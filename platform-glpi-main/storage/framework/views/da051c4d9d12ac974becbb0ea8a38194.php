    ...
    <div class="card shadow-lg border-radius-lg p-3"
      ...
      <div class="d-flex align-items-center">
        ...
        <div>
          ...
          <p class="text-white text-sm mb-0 opacity-8"><?php echo e($countAll); ?> clients enregistrés</p>
        </div>
      </div>
    </div>
  </div>
...
<div class="card">
  <div class="card-body p-3">
    ...
    <div class="d-flex align-items-start justify-content-between flex-wrap gap-3 mb-3">
      <div>
        <div class="seg-tabs">
          <a href="<?php echo e(route('admin.clients')); ?>"
             class="seg-tab <?php echo e($activeFilter === 'all' || !request('client_type') ? 'active' : ''); ?>">
            <i class="material-symbols-rounded" style="font-size:16px;">group</i>
            Tous <span class="tab-count"><?php echo e($countAll); ?></span>
          </a>
          <a href="<?php echo e(route('admin.clients')); ?>?client_type=client"
             class="seg-tab <?php echo e($activeFilter === 'client' ? 'active' : ''); ?>">
            <i class="material-symbols-rounded" style="font-size:16px;">verified_user</i>
            Client <span class="tab-count"><?php echo e($countClient); ?></span>
          </a>
          <a href="<?php echo e(route('admin.clients')); ?>?client_type=user"
             class="seg-tab <?php echo e($activeFilter === 'user' ? 'active' : ''); ?>">
            <i class="material-symbols-rounded" style="font-size:16px;">new_releases</i>
            Nouveau <span class="tab-count"><?php echo e($countNew); ?></span>
          </a>
        </div>
      </div>
      ...
      <div class="cl-search" style="min-width:240px;max-width:320px;">
        ...
        <button id="clearBtn" type="button" onclick="clearSearch()"
                style="background:none;border:none;cursor:pointer;padding:0;display:<?php echo e(request('search') ? 'block' : 'none'); ?>">
    ...
    <p class="text-xs text-secondary mb-2 ps-1">
      <?php echo e($clients->total()); ?> client(s)
      <?php if($activeFilter === 'client'): ?> &middot; <span style="color:#6D28D9;font-weight:700;">🟣 Clients classifiés</span>
      <?php elseif($activeFilter === 'user'): ?> &middot; <span style="color:#C2410C;font-weight:700;">🟠 Non classifiés</span>
      <?php endif; ?>
    </p>
    ...
    <div style="border:1px solid #f1f5f9;border-radius:12px;overflow:hidden;">
      ...
      ?>
      <div class="cl-item" onclick="window.location.href='<?php echo e(route('admin.clients.show', $client->id)); ?>'" style="cursor:pointer;">
        <div class="cl-avatar">
          <?php if($client->avatar): ?>
            <img src="<?php echo e(asset('storage/'.$client->avatar)); ?>" style="width:100%;height:100%;object-fit:cover;" alt="">
          <?php else: ?>
        ...
        <div class="cl-info">
          <div class="cl-name"><?php echo e($client->name); ?></div>
          <div class="cl-email"><?php echo e($client->email); ?></div>
        </div>
        <div class="cl-right">
          <span class="ctype ctype-<?php echo e($ctypeCss); ?>"><?php echo e($ti['icon']); ?> <?php echo e($ti['label']); ?></span>
          <div class="cl-tickets"><?php echo e($client->tickets_count); ?> ticket(s)</div>
        </div>
      </div>
      ...
      <div class="text-center py-5">
        ...
        <?php if(request('search') || request('client_type')): ?>
          <a href="<?php echo e(route('admin.clients')); ?>" class="btn btn-sm btn-outline-secondary">Réinitialiser</a>
        <?php endif; ?>
      </div>
      <?php endif; ?>
    </div>

    <div class="pt-3"><?php echo e($clients->appends(request()->query())->links()); ?></div>
  </div>
</div>
...
var CLIENTS_DATA = {
  ...
  <?php echo e($c->id); ?>: {
    id:           <?php echo e($c->id); ?>,
    name:         <?php echo json_encode($c->name, 15, 512) ?>,
    email:        <?php echo json_encode($c->email, 15, 512) ?>,
    phone:        <?php echo json_encode($c->phone ?? null, 15, 512) ?>,
    phone_mobile: <?php echo json_encode($c->phone_mobile ?? null, 15, 512) ?>,
    whatsapp:     <?php echo json_encode($c->whatsapp ?? null, 15, 512) ?>,
    teams_email:  <?php echo json_encode($c->teams_email ?? null, 15, 512) ?>,
    type:         <?php echo json_encode($c->client_type, 15, 512) ?>,
    glpi_id:      <?php echo json_encode($c->glpi_user_id ?? null, 15, 512) ?>,
    tickets:      <?php echo e($c->tickets_count); ?>,
    active:       <?php echo e($c->is_active ? 'true' : 'false'); ?>,
    date:         <?php echo json_encode($c->created_at->format('d/m/Y'), 15, 512) ?>,
    last_login:   <?php echo json_encode($c->last_login_at ? \Carbon\Carbon::parse($c->last_login_at)->format('d/m/Y H:i') : null, 15, 512) ?>,
    avatar:       <?php echo json_encode($c->avatar ? asset('storage/'.$c->avatar) : null, 15, 512) ?>,
    first_name:   <?php echo json_encode($c->first_name ?? null, 15, 512) ?>,
    last_name:    <?php echo json_encode($c->last_name ?? null, 15, 512) ?>,
    birthday:     <?php echo json_encode($c->birthday ? \Carbon\Carbon::parse($c->birthday)->format('d/m/Y') : null, 15, 512) ?>,
    age:          <?php echo json_encode($c->birthday ? \Carbon\Carbon::parse($c->birthday)->age : null, 15, 512) ?>,
    gender:       <?php echo json_encode($c->gender ?? null, 15, 512) ?>,
    hasChat:      <?php try { echo \DB::table('chat_access_grants')->where('admin_id',auth()->id())->where('client_id',$c->id)->exists() ? 'true' : 'false'; } catch(\Exception $e) { echo 'false'; } ?>,
  },
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/resources/views/admin/clients.blade.php ENDPATH**/ ?>