...
function getFapiId(id) {
  ...
  return t ? (t.fastapi_ticket_id || t.id || null) : null;
}
...
function applyOverride() {
  ...
  var fapiId   = getFapiId(S.selectedId);
  if (!fapiId) {
    toast('Impossible de trouver l\u2019ID de ticket support.');
    return;
  }
  ...
  .catch(function (err) {
    document.getElementById('applyBtn').disabled = false;
    toast('Erreur override : ' + (err.message || 'Erreur'));
  });
}
