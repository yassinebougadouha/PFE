...
# settings = get_voice_settings() # Moved inside functions to be import-safe
...
async def entrypoint(ctx: agents.JobContext):
    ...
    # Capture live turns as a fallback when post-call audio transcription is unavailable.
    ...
    def _on_local_track_published(_publication, track: rtc.Track):
        audio_recorder.on_local_track_published(track)
    ...
    async def _on_shutdown():
        ...
        logger.info("Voice session ending — finalizing transcript & audio for room=%s", room_name)
        ...
        try:
            ...
            if not french_transcript:
                if audio_path:
                    logger.warning(
                        "Audio transcription returned empty transcript for room=%s, falling back to live turns.",
                        room_name,
                    )
                else:
                    logger.warning(
                        "No recording available for room=%s, falling back to live transcript events.",
                        room_name,
                    )
        ...
        # 3. Retrieve escalation state
        state = shared_state.pop_session_state(room_name)
        ...
        try:
            ...
            logger.debug("Failed to clear support-call screen context for room=%s: %s", room_name, exc)

        # 4. Persist to database
        if french_transcript or audio_path or is_escalated:
            try:
                ...
                logger.info("VoiceCallLog saved to DB for room=%s (escalated=%s)", room_name, is_escalated)
            except Exception as exc:
                logger.error("Failed to save VoiceCallLog for room=%s: %s", room_name, exc)
...
async def _save_call_log(
    ...
    try:
        ...
        from app.db.models.enums import UserRole, UserStatus, TicketPriority, ChannelType, TicketStatus
        ...
        if transcript:
            logger.info("=== TRANSCRIPT (room=%s) ===\n%s\n=== END ===", room_name, transcript)
        return

    settings = get_voice_settings()
    db_url = settings.database_url
    if not db_url:
        logger.warning("DATABASE_URL not set — cannot save VoiceCallLog. Logging transcript to console.")
        if transcript:
            logger.info("=== TRANSCRIPT (room=%s) ===\n%s\n=== END ===", room_name, transcript)
        return
    ...
    async with async_session() as session:
        ...
        if is_escalated:
            ...
            user_res = await session.execute(select(User).where(User.email == system_email))
